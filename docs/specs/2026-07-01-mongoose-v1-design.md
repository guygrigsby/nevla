...
Two binaries, split like uv and python. `mongoose` is the setup and project tool; `tk` is the runner. No pip, no venv, no PATH archaeology. `mongoose` drives `uv` under the hood for all Python provisioning rather than reimplementing any of it.
...
tk script.mg            # run a file, python-style
tk                      # repl
```

Scripts are executable: a leading `#!/usr/bin/env tk` line is skipped by the lexer, so `chmod +x` works. Bare `mongoose run` and `mongoose check` resolve the nearest project's `src/main.mg`.
...
- The repl (`tk` with no file) is unchecked in v1: lines go straight to the evaluator, faults are reported and survived.
