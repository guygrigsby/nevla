...
Not on PyPI yet: until the first release, build the wheel from a checkout (needs a Rust toolchain): `uvx maturin build --release`, then `uv tool install target/wheels/rikki_lang-*.whl`.
