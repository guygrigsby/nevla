...
pub(super) const STD_MODULES: &[&str] =
    &["math", "error", "file", "ctx", "gpu", "http", "test", "time", "os", "regex", "flag", "proc"];
...
impl Checker {
    pub(super) fn container_method(
        ...
        match (recv, name) {
            ...
            (Str, "repeat") => {
                self.check_args(&[Int], args, span);
                one(Str)
            }
            (List(elem), "map") => {
                let f = Fn(vec![(**elem).clone()], vec![Unknown]);
                let got = self.args_with_fn(&f, args, span);
                let out = match got {
            ...
            (List(elem), "sum") => {
                ...
                if !matches!(**elem, Int | Float | Unknown) {
                    self.diag(span, format!("sum needs []int or []float, got {recv}"));
                }
                one((**elem).clone())
            }
            (List(elem), "sorted") => {
            ...
            (List(elem), "contains") => {
                self.check_args(&[(**elem).clone()], args, span);
                one(Bool)
            }
            (List(elem), "join") => {
                if !matches!(**elem, Str | Unknown) {
                    self.diag(span, format!("join needs []str, got {recv}"));
                }
                self.check_args(&[Str], args, span);
                one(Str)
            }
